( function connectjs() {
	...
	function getDefaultOptions() {
		return {
			...
			clientServerFinishUrl:
				'https://yourserver.com/api/v1/quantimodo/finish',
			finish: function ( sessionTokenObject ) {
				logError(
					'You have not defined window.QuantiModoIntegration.options.finish!'
				);
				console.warn(
					'window.QuantiModoIntegration.options.finish is called after user finishes connecting their health data.'
				);
				console.warn(
					'You should set this to POST sessionTokenObject as-is to your server for step 2'
				);
				...
				req.open(
					'POST',
					getApiOrigin() + '/api/v1/quantimodo/connect/finish'
				);
	...
	for ( var key in qmPageElements ) {
		if ( qmPageElements.hasOwnProperty( key ) ) {
			qmPageElements[ key ].id = camelCaseToDash( key );
		}
	}
	...
	function logError( errorMessage ) {
		...
		if ( window.QuantiModoIntegration.error ) {
			window.QuantiModoIntegration.error( errorMessage );
		}
		if ( getDefaultOptions().error ) {
			getDefaultOptions().error( errorMessage );
		}
	}
	function dashesToCamelCase( myString ) {
	...
	function getClientId() {
		var clientId;
		if ( typeof options !== 'undefined' && options.clientId ) {
			clientId = options.clientId;
		}
		if ( ! clientId ) {
			clientId = window.QuantiModoIntegration.options.clientId;
		}
		if ( ! clientId ) {
			clientId = 'quantimodo';
		}
	...
	function getApiHost() {
		...
		if ( window.location.href.indexOf( 'staging.quantimo.do' ) !== -1 ) {
			apiUrl = 'staging.quantimo.do';
		}
		if ( window.location.href.indexOf( 'qm-staging.quantimo.do' ) !== -1 ) {
			apiUrl = 'staging.quantimo.do';
		}
	...
	function getAppSettings( successHandler ) {
		...
		function getAppSettingsUrl() {
			return (
				'https://' +
				getApiHost() +
				'/api/v1/appSettings?clientId=' +
				getClientId()
			);
		}
		...
		xhr.onreadystatechange = function () {
			if ( xhr.readyState === 4 ) {
				...
				if (
					window.QuantiModoIntegration.appSettings &&
					typeof window.QuantiModoIntegration.appSettings !==
						'undefined'
				) {
					console.log(
						'got appSettings',
	...
	function createNewDiv( qmPageElement ) {
		...
		if ( document.getElementById( qmPageElement.id ) ) {
			return;
		}
		...
		if ( qmPageElement.onClickListener ) {
			qmPageElement.element.addEventListener(
				'click',
				qmPageElement.onClickListener
			);
		}
	}
	...
	function getAPIQueryString() {
		var queryString;
		var moreInfo =
			'  See https://builder.quantimo.do for more information.';
		if ( getClientId() && getClientId() !== '' ) {
			queryString = '?clientId=' + getClientId();
		} else {
			throw (
				'Please set window.QuantiModoIntegration.options.clientId! ' +
				moreInfo
			);
		}
		...
		if ( qmAccessToken && qmAccessToken !== '' ) {
			...
			window.QuantiModoIntegration.options.publicToken &&
			window.QuantiModoIntegration.options.publicToken !== ''
		) {
			queryString +=
				'&publicToken=' +
			...
			console.warn(
				'No window.QuantiModoIntegration.options.publicToken or window.QuantiModoIntegration.options.qmAccessToken provided so assuming this a new user. ' +
					moreInfo
			);
		}
		if (
			window.QuantiModoIntegration.options.clientUserId &&
			window.QuantiModoIntegration.options.clientUserId !== ''
		) {
			queryString +=
				'&clientUserId=' +
			...
			console.warn(
				'Please set window.QuantiModoIntegration.options.clientUserId! ' +
					moreInfo
			);
		}
		return queryString;
	...
	function postJsonToAPI( jsonObject, url, successHandler, errorHandler ) {
		...
		xhr.onreadystatechange = function () {
			if (
				...
				if ( errorHandler ) {
					errorHandler( url + ' response: ' + xhr.responseText );
				}
			}
		};
	...
	function connectorsApiCall( method, requestPath, success, errorHandler ) {
		...
		request.open(
			method,
			'https://' + getApiHost() + requestPath + getAPIQueryString(),
			true
		);
		request.onload = function () {
			if ( request.status >= 200 && request.status < 400 ) {
				...
				success( data );
				if (
					window.QuantiModoIntegration.options.finish &&
	...
	function loadConnectors( callback ) {
		connectorsApiCall( 'GET', '/api/v3/connectors/list', function ( data ) {
			...
			data.connectors.forEach( function ( connector ) {
				connectorList[ connector.name + connector.id ] = connector;
			} );
			callback();
		} );
	}
	function renderConnectorList() {
		...
		function showAuthWindow( url ) {
			...
			windowSize.left =
				window.screenX + ( window.outerWidth - windowSize.width ) / 2;
			windowSize.top =
				window.screenY + ( window.outerHeight - windowSize.height ) / 8;
			var windowOptions =
				'width=' + windowSize.width + ',height=' + windowSize.height;
			windowOptions +=
				',toolbar=0,scrollbars=1,status=1,resizable=1,location=1,menuBar=0';
			windowOptions +=
				',left=' + windowSize.left + ',top=' + windowSize.top;
		...
		function addFieldsFromConnectButton( button, block, instructions ) {
			for ( var i in instructions.parameters ) {
				if ( instructions.parameters.hasOwnProperty( i ) ) {
					...
					var fields = block.querySelector(
						'.qm-account-block-fields'
					);
					...
					input.addEventListener( 'keypress', function ( event ) {
						if ( event.keyCode === 13 ) {
							...
							var connectButton = block.getElementsByClassName(
								'qm-account-connect-button'
							)[ 0 ];
							if ( connectButton ) {
								connectButton.click();
							}
		...
		var connectorBlock = {
			template:
				'<div class="qm-account-block" data-name="<%= name %><%= id %>">' +
		...
		for ( var key in connectorList ) {
			if ( connectorList.hasOwnProperty( key ) ) {
				renderedBlock = template(
					connectorBlock.template,
					connectorList[ key ]
				);
		...
		delegateSelector(
			...
			function ( event ) {
				...
				var hasParameters = countObjectAttributes(
				...
				if ( useConnectionWindow && ! hasParameters ) {
					...
					if (
						...
						! hasClass(
							event.target,
							'qm-account-connect-button-with-params'
						)
					) {
						addFieldsFromConnectButton(
							event.target,
							block,
							instructions
						);
		...
		delegateSelector(
			...
			function ( event ) {
				...
				connectorsApiCall(
					'GET',
					'/api/v1/connectors/' + name + '/disconnect',
					function () {
	...
	function getIonicUrl( statePath ) {
		return (
			getDefaultOptions().baseIonicPath +
			statePath +
			window.QuantiModoIntegration.getIframeQueryString()
		);
	}
	...
	window.QuantiModoIntegration.qmSetupIonicPopupIframe = function ( state ) {
		function createQmShowHideButtonBlock() {
			...
			showElement(
				qmPageElements.singleFloatingActionButton.element,
				5000
			);
		}
		function createHiddenIframePopupBlock() {
			...
			qmPageElements.qmPopupInner =
				document.getElementById( 'qm-popup-inner' );
			qmPageElements.singleFloatingActionButton =
				document.getElementById( 'qm-close' );
			qmPageElements.singleFloatingActionButton.addEventListener(
				'click',
				function () {
		...
		window.onload = function () {
			...
			qmMain.innerHTML =
				'<iframe style="height:100%;width:100%;" id="ionic-app-frame" src="' +
				url +
	...
	window.QuantiModoIntegration.openConnectorsListPopup = function () {
		...
		if ( ! window.QuantiModoIntegration.options.fullscreen ) {
			connectorListCssStyles +=
				'#qm-popup-inner {max-width:900px; min-width:600px; padding:10px 50px border:2px solid gray; border-radius:10px; position: relative;}';
			connectorListCssStyles +=
				'#qm-close {position:absolute; right:-14px; top:-14px; cursor:pointer;}';
		} else {
			connectorListCssStyles +=
				'#qm-popup-inner {padding: none; width: 100%;}';
			connectorListCssStyles +=
				'#qm-close {position:absolute; right:40px; top:40px; cursor:pointer;}';
		}
		connectorListCssStyles += ';';
	...
	window.QuantiModoIntegration.renderConnectorListAtProvidedSelector =
		function ( connectOptions ) {
			window.onload = function () {
				var selector = connectOptions.selector
					? connectOptions.selector
					: connectOptions;
	...
	window.QuantiModoIntegration.renderConnectorPageForMobileEmbed =
		function () {
			window.onload = function () {
				useConnectionWindow = false;
	...
	function toggleQmIonicAppSidebar() {
		function openAppSidebar() {
			...
			qmPageElements.singleFloatingActionButton.element.setAttribute(
				'style',
				qmPageElements.singleFloatingActionButton.css.open
			);
		}
		function closeAppSidebar() {
			...
			qmPageElements.singleFloatingActionButton.element.setAttribute(
				'style',
				qmPageElements.singleFloatingActionButton.css.closed
			);
		}
	...
	window.QuantiModoIntegration.getIframeQueryString = function () {
		...
		if ( window.QuantiModoIntegration.options.qmAccessToken ) {
			str +=
				'&accessToken=' +
				window.QuantiModoIntegration.options.qmAccessToken;
		}
		if ( window.QuantiModoIntegration.options.hideMenu ) {
			str += '&hideMenu=' + window.QuantiModoIntegration.options.hideMenu;
		}
		if ( window.QuantiModoIntegration.options.logout ) {
			str += '&logout=' + window.QuantiModoIntegration.options.logout;
		}
		if ( window.QuantiModoIntegration.options.clientId ) {
	...
	window.QuantiModoIntegration.createSingleFloatingActionButton =
		function () {
			getAppSettings( function () {
				...
				var statePath =
					window.QuantiModoIntegration.options.defaultState ||
					'intro';
				qmPageElements.qmIonicAppSidebar.template =
					'<div id="' +
					qmPageElements.qmIonicAppSidebar.id +
					...
					';" id="ionic-app-frame" frameborder="0" ' +
					'src="' +
					getIonicUrl( statePath ) +
					'">' +
					'</iframe>' +
					'</div>';
				...
				function getIconUrl() {
					...
					console.log(
						'window.QuantiModoIntegration.appSettings.additionalSettings',
						window.QuantiModoIntegration.appSettings
							.additionalSettings
					);
					console.log(
						...
						window.QuantiModoIntegration.appSettings
							.additionalSettings.appImages
					);
					if ( window.QuantiModoIntegration.appSettings ) {
						return window.QuantiModoIntegration.appSettings
							.additionalSettings.appImages.appIcon;
					}
					return (
						'https://static.quantimo.do/app_uploads/' +
						getClientId() +
						'/app_images_appIcon.png'
					);
				}
				qmPageElements.singleFloatingActionButton.template =
					...
					qmPageElements.singleFloatingActionButton.css.closed +
					'display:none;" id="' +
					qmPageElements.singleFloatingActionButton.id +
					'" src="' +
					getIconUrl() +
					'"/>';
				qmPageElements.singleFloatingActionButton.onClickListener =
					function () {
						console.debug( 'Clicked QM button' );
	...
	window.QuantiModoIntegration.createTripleFloatingActionButton =
		function () {
			var tripleFloatingActionButtonsCssStyles =
				'.qm-fab-container { bottom: 0; position: fixed; margin: 1em; right: ' +
				qmPageElements.tripleFloatingActionButtons.right +
				'px;}' +
			...
			qmPageElements.tripleFloatingActionButtons.template =
				'<nav id="' +
				qmPageElements.tripleFloatingActionButtons.id +
				'" class="qm-fab-container"> ' +
	...
	function addEventListenerToQmIntegrationButton() {
		window.onload = function () {
			var qmIntegrationButtonElement = document.getElementById(
				'qm-integration-button'
			);
			if ( qmIntegrationButtonElement ) {
				createNewDiv( qmPageElements.qmIonicAppSidebar );
				qmIntegrationButtonElement.addEventListener(
					'click',
					toggleQmIonicAppSidebar
				);
			}
