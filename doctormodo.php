<?php
/*
Template Name: doctormodo
*/
?>

<HTML><HEAD>
<META NAME="Description" CONTENT="Talk to Dr. Modo!">
<TITLE>Dr. Modo</TITLE>
...
// Wait function to allow our pieces to get here prior to starting
        ...
        function hello(){
                chatter[chatpoint] = "> Hello, I am Dr. Modo."; 
                chatpoint = 1;
                return write();
        }
        function start(){
...
// And to give the illusion that some thinking is going on
        ...
        function think(){
                document.DrModo.input.value = "";        
                if( DrModoresponse != "" ){ respond(); }                
                else { setTimeout("think()", 250); }
        }
        function dialog(){
                var Input = document.DrModo.input.value;          // capture input and copy to log
                document.DrModo.input.value = "";        
                chatter[chatpoint] = " \n* " + Input;
                DrModoresponse = listen(Input);
...
// Allow for unprompted response from the engine
        ...
        function write(){
                document.DrModo.log.value = "";
                for(i = 0; i < chatmax; i++){
                        n = chatpoint + i;
                        if( n < chatmax ){ document.DrModo.log.value += chatter[ n ]; }
                        else { document.DrModo.log.value += chatter[ n - chatmax ]; }
                }
                refresh();
                return false;                              // don't redraw the DrModo's form!
        }
...
// --></SCRIPT>
<link rel="stylesheet" href="https://www.manifestation.com/station.css">
</HEAD>
...
<table cellspacing="0" cellpadding="2" border="0" align="center" class="neurotoy">
        <tr><th bgcolor=#000000 align="left">
                <font color="#FFFFFF">Talk to Dr. Modo</Font>
        </td></tr>
          <tr><td bgcolor=#0097FF>
            <form name="DrModo" onSubmit="return dialog();">
